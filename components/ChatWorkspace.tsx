...
const suggestions = [
  {
    ...
    prompt:
      "Ich suche eine deutschsprachige React-Entwicklerin oder einen React-Entwickler für ein Remote-Projekt. Bitte hilf mir, passende Freelancer zu finden.",
  },
  {
    label: "Anforderungsmanagement",
    description: "Anforderungen strukturieren und begleiten",
    prompt:
      "Ich suche kurzfristig Unterstützung im Anforderungsmanagement für ein B2B-Softwareprojekt. Bitte finde passende Expertinnen oder Experten.",
  },
  {
    label: "Prozessmanagement",
    description: "Abläufe analysieren und verbessern",
    prompt:
      "Wir benötigen einen erfahrenen Freelancer für Prozessmanagement und die Optimierung interner Abläufe. Bitte zeige passende Profile.",
  },
  {
    label: "Informationssicherheit",
    description: "Expertise für sichere Organisationen",
    prompt:
      "Ich suche eine Expertin oder einen Experten für Informationssicherheit, idealerweise mit Erfahrung in mittelständischen Unternehmen.",
  },
] as const;
        ...
        <div className="chat-scroll" aria-live="polite">
          <div className="conversation">
            {messages.length === 0 && !pendingAssistant ? (
              <WelcomeState onSuggestion={(prompt) => void sendMessage(prompt)} />
            ) : (
        ...
        <div className="composer-zone">
          ...
          <form className="composer" onSubmit={handleSubmit}>
            ...
            <textarea
              ...
              placeholder={messages.length ? "Weitere Informationen hinzufügen …" : "Welchen Freelancer suchen Sie?"}
              rows={1}
              maxLength={12_000}
            />
    ...
    <section className="welcome-state" aria-labelledby="welcome-title">
      ...
      <div className="suggestion-grid" aria-label="Beispielanfragen">
        {suggestions.map((suggestion) => (
          <button key={suggestion.label} type="button" onClick={() => onSuggestion(suggestion.prompt)}>
  ...
  return (
    <article className="brief-card">
      <div className="brief-header">
        <div>
          <p className="eyebrow">Verstandene Anfrage</p>
      ...
      <dl className="brief-grid">
        ...
        <DetailTerm label="Arbeitsmodus" value={brief.mode === "unknown" ? null : modeLabel(brief.mode)} />
        <DetailTerm label="Start & Dauer" value={[brief.startWindow, brief.duration].filter(Boolean).join(" · ") || null} />
      </dl>
      {brief.unknownFields.length ? (
        <div className="unknown-row"><span>Offen</span>{brief.unknownFields.join(" · ")}</div>
      ) : null}
    </article>
  );
}
